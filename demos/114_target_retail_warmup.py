...
"""Demo: Target retail warmup — browse deals and categories."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("TARGET_SCROLLS", "6"))
    banner("Target Retail Warmup", f"Deals browse  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.target.com/", wait_until="domcontentloaded", timeout=90000)
