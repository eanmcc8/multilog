...
"""Demo: Uber Eats delivery warmup — browse restaurants and menus."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("UBEREATS_SCROLLS", "5"))
    banner("Uber Eats Warmup", f"Restaurant browse  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.ubereats.com/", wait_until="domcontentloaded", timeout=90000)
