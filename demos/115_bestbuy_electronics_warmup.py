...
"""Demo: Best Buy electronics warmup — browse products and deals."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("BESTBUY_SCROLLS", "5"))
    banner("Best Buy Electronics Warmup", f"Product browse  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.bestbuy.com/", wait_until="domcontentloaded", timeout=90000)
