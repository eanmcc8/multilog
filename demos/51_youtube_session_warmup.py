...
"""Demo: YouTube session warmup — natural watch pattern for profile trust."""
import asyncio
import os
import random
...
async def main():
    ...
    banner("YouTube Session Warmup", f"Natural watch pattern  |  {timestamp()}")
    warn("For your own channels/content only — artificial watch inflation violates YouTube ToS")
    ...
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            ...
            ok("YouTube warmup session complete")
