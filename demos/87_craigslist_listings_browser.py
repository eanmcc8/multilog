...
"""Demo: Craigslist listings browser — browse local classifieds."""
import asyncio
import os
import random
...
async def main():
    ...
    city = os.getenv("CRAIGSLIST_CITY", "https://newyork.craigslist.org")
    scrolls = int(os.getenv("CRAIGSLIST_SCROLLS", "5"))
    banner("Craigslist Listings Browser", f"{city}  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto(city, wait_until="domcontentloaded", timeout=60000)
