...
"""Demo: Kraken crypto exchange warmup — browse markets."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("KRAKEN_SCROLLS", "5"))
    banner("Kraken Crypto Warmup", f"Market browse  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.kraken.com/", wait_until="domcontentloaded", timeout=60000)
