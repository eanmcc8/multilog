...
"""Demo: Canva design browser — browse templates and dashboard."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("CANVA_SCROLLS", "5"))
    banner("Canva Design Browser", f"Template browse  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.canva.com/", wait_until="domcontentloaded", timeout=90000)
            await human_delay(4000, 7000)
            if "login" in page.url.lower():
                warn("Canva login optional for template gallery")
