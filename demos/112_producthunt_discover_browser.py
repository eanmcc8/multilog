...
"""Demo: Product Hunt discover browser — browse daily product launches."""
import asyncio
import os
import random
...
async def main():
    ...
    scrolls = int(os.getenv("PH_SCROLLS", "6"))
    banner("Product Hunt Discover", f"Launch feed  |  {timestamp()}")
    mlx = MultiloginX()
    async with async_playwright() as pw:
        async with profile_browser(mlx, env["MLX_PROFILE_ID"], env["MLX_FOLDER_ID"], pw) as session:
            page = await session.new_page()
            await page.goto("https://www.producthunt.com/", wait_until="domcontentloaded", timeout=60000)
