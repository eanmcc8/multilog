...
"""Demo: Udemy course browser — search and browse course catalog."""
import asyncio
import os
import random
...
async def main():
    ...
    query = os.getenv("UDEMY_QUERY", "automation")
    scrolls = int(os.getenv("UDEMY_SCROLLS", "5"))
    banner("Udemy Course Browser", f"Query: {query}  |  {timestamp()}")
    mlx = MultiloginX()
