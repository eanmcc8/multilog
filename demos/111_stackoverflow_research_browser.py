...
"""Demo: Stack Overflow research browser — search dev Q&A."""
import asyncio
import os
import random
...
async def main():
    ...
    query = os.getenv("SO_QUERY", "playwright python")
    scrolls = int(os.getenv("SO_SCROLLS", "5"))
    banner("Stack Overflow Research", f"Query: {query}  |  {timestamp()}")
    mlx = MultiloginX()
